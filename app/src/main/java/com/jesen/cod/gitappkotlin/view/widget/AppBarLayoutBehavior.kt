...
class AppBarLayoutBehavior(context: Context, attrs: AttributeSet?) :
    ...
    init {
        if (attrs == null) {
            ...
            targetTop =
                a.getDimensionPixelSize(R.styleable.AppBarLayoutBehavior_targetTop, INVALID_VALUE)
            targetLeft =
                a.getDimensionPixelSize(R.styleable.AppBarLayoutBehavior_targetLeft, INVALID_VALUE)
            originalTop =
                a.getDimensionPixelSize(R.styleable.AppBarLayoutBehavior_originalTop, INVALID_VALUE)
            originalLeft = a.getDimensionPixelSize(
                R.styleable.AppBarLayoutBehavior_originalLeft,
                INVALID_VALUE
            )
            a.recycle()
        }
    ...
    private fun initializeProperties(child: View, appBarLayout: AppBarLayout) {
        ...
        if (targetLeft == INVALID_VALUE) {
            targetLeft =
                (originalLeft + (originalWidth - targetWidth) * child.pivotX / originalWidth).toInt()
        }

        if (targetTop == INVALID_VALUE) {
            targetTop =
                (originalTop + (originalHeight - targetHeight) * child.pivotY / originalHeight).toInt()
        }

        totalOffsetX =
            (targetLeft - originalLeft + (targetWidth - originalWidth) * child.pivotX / originalWidth).toInt()
        totalOffsetY =
            (targetTop - originalTop + (targetHeight - originalHeight) * child.pivotY / originalHeight).toInt()
    }
